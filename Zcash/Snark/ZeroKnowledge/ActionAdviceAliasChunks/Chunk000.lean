...
private noncomputable def piece000 :
    AdviceMapScanPiece adviceAliasMapStep ∅
      (adviceAliasAddressData (actionAdviceSourceCertificate.annotations.map (fun entry => (entry.1.instruction, entry.2)))) := by
...
noncomputable def actionAdviceAliasChunk000 :
    AdviceMapScanPiece adviceAliasMapStep ∅
