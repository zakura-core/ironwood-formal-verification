...
**`scripts/regenerate-fingerprint-fixtures.sh`** checks that committed captures
regenerate byte-for-byte from Zakura's pinned Common release. It authenticates the
source archives and published crates, uses the release lockfile/toolchain, runs the
four existing capture drivers, and compares all six artifacts. CI runs the full
...
`--check-set` performs only the local inventory check. `--from-run` checks a
completed capture run without executing Rust again; `--update` installs its
generated bytes and updates both provenance manifests. CI only compares artifacts.
The Lean build is separate, and the checked-in captures retain their recorded
origin until a successful refresh updates them.
